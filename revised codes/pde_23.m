function f = pde_23(t,y,N,V_medium,D,dx,Bmaxc,n,ICN,v)
v =0;
Bmaxm = 3.94*10^3;
Bmaxc = 6300;
Kbar=781;
...
for i = 1:n
    if i == 1 % BC at x = 0
        ...
        C_ist(i) = k_fast*k1*(B(i)-C_is(i))*C_if(i) - k_fast*k2*C_is(i) - Pt(i)*C_is(i)/P(i);
        C_inst(i) = k_fast*k3*C_if(i) - k_fast*k4*C_ins(i) - Pt(i)*C_ins(i)/P(i);
        C_nt(i) = 0;
        T_tot(i) = a1 - a4*(T_tot(i)-B(i));
    else 
        Bt(i) = (b1+b2*C_is(i))*(T_tot(i)-B(i)) - 2*b1*B(i) - gamma*C_n(i)*B(i);
