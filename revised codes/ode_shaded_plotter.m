...
k3 = 350 + 20*randn(n,1);
k4 = 10 + 2*randn(n,1);
k5 = 0.076 + 0.01*randn(n,1);
k6 = 0.1 + 0.02*randn(n,1);
alpha = 2.7*10^(-3) + 10^(-4)*randn(n,1);
a3 = 12*ones(n,1);
a2 = zeros(n,1);
a1 = a3*(T0-B0);
V_medium = 1000;
...
b2 =0.0023 + 10^(-4)*randn(n,1);
b3 = 0.01 + 10^(-3)*randn(n,1);
b1 = 21.6*ones(n,1);
...
file_name = [save_file 'odeuptake.png'];
...
%title('time series data for incubation (100 nM)','fontsize',16)
file_name = [save_file 'odeuptake2hr.png'];
...
hold on
plot_distribution(concs,m2','color',[0 0 1])
hold on
plot_distribution(concs,m3','color',[0 0 0])
hold on
plot(K(:,1),K(:,2),'rx')
hold on
plot(K(:,1),K(:,3),'bx')
...
file_name = [save_file 'odevol.png'];
...
hold on
plot_distribution(1000*concs, washout_mod);
hold on
plot_distribution(1000*concs, eq_mod);
hold on
...
file_name = [save_file 'odeintra.png'];
...
ylabel('intracellular concentration (\mu M)','fontsize',16)
file_name = [save_file 'ODEwashtimeseries.png'];
...
Cn =0;
ICN = 0;
y_0 = [ICN 0 0 0 150 0 T0 B0];
g = zeros(8,length(t),n);
for i = 1:n
sol = ode15s(@model1T3, [0 24], y_0, [], P1{i,:}, k_cellnumber,gamma,Cn,V_medium);
g(:,:,i) = deval(sol,t);
end
